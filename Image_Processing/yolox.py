...
from Image_Processing.nets.yolo import YoloBody
...
class YOLO(object): 
    ...
    def detect_image(self, image, crop = False):
        ...
        #   设置字体与边框厚度
        #---------------------------------------------------------#
        font        = ImageFont.truetype(font='./model/simhei.ttf', size=np.floor(3e-2 * image.size[1] + 0.5).astype('int32'))
        ...
        if crop:
            for i, c in list(enumerate(top_label)):
                ...
                crop_image.save(os.path.join(dir_save_path,  image.filename.split('/')[-1]),  quality=95, subsampling=0)
